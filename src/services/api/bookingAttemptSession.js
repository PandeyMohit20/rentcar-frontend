...
const readAttempt = () => {
  try {
    return JSON.parse(window.sessionStorage.getItem(ATTEMPT_KEY))
  } catch {
    return null
  }
}
...
const writeRecovery = (value) => {
  ...
  try {
    ...
    /* Memory recovery still works for this tab. */
  }
  return value
}
...
export const bookingAttemptSession = {
  getOrCreateKey(contextKey) {
    ...
    if (!preCreateAttempt || preCreateAttempt.contextKey !== contextKey) {
      ...
      try {
        window.sessionStorage.setItem(ATTEMPT_KEY, JSON.stringify(preCreateAttempt))
      } catch {
        /* Keep the key stable in memory. */
      }
    }
