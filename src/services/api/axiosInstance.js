...
const publicAuth = (url) =>
  /^\/auth\/(login|register|forgot-password|reset-password|verify-email|resend-verification|send-otp|verify-otp)$/.test(
    url || ''
  )
function normalize(error) {
  ...
  let message = isTimeout
    ? 'The request timed out. Check the latest status before trying again.'
    : isNetworkError
  ...
  if (status === 413 || data?.error?.code === 'LIMIT_FILE_SIZE')
    message = 'File is too large. Choose a file up to 10 MB.'
  ...
  return {
    ...
    data:
      data && !(data instanceof Blob)
        ? { message, error: { code: data.error?.code, details: data.error?.details } }
        : null,
  }
}
...
function subject(token) {
  try {
    return JSON.parse(atob(token.split('.')[1].replaceAll('-', '+').replaceAll('_', '/'))).sub
  } catch {
    return null
  }
}
...
function refreshAccessToken(version) {
  if (!refreshPromise || refreshPromise.version !== version) {
    const promise = axios
      .post(baseURL + '/auth/refresh', {}, { withCredentials: true, timeout })
      .then((response) => {
        const token = response.data?.data?.accessToken
        const priorSubject = subject(authSession.getAccessToken())
        if (priorSubject && subject(token) !== priorSubject) {
          expire(version)
...
axiosInstance.interceptors.response.use(
  (response) => {
    if (response.config._sessionVersion !== authSession.getVersion())
  ...
  async (error) => {
    ...
    if (version !== undefined && version !== authSession.getVersion())
      return Promise.reject(sessionChanged())
    if (
      error.response?.status === 401 &&
      request &&
      !publicAuth(request.url) &&
      ...
      try {
        ...
        const token =
          current && request.headers.Authorization !== 'Bearer ' + current
            ? current
